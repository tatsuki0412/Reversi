...
public class GameSession {
    ...
    public synchronized boolean makeMove(int row, int col, char player) {
        if (player != currentPlayer) return false; // Not this player's turn.
    ...
    public boolean isValidMove(int row, int col, char player) {
        if (!isWithinBounds(row, col) || board[row][col] != '.') return false;
        char opponent = (player == 'B') ? 'W' : 'B';
        for (int[] d : DIRECTIONS) {
            int r = row + d[0], c = col + d[1];
            if (!isWithinBounds(r, c) || board[r][c] != opponent) continue;
            r += d[0];
            c += d[1];
    ...
    // Send the initial messages to both players and update them with the board state.
